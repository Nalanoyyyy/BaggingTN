  ...
  background-color: ${({ status }) => {
    switch (status) {
      ...
      case 'finish,':
        return '#4caf50'; // เขียว
       default:
        return '#ffb300'; // เทา สำหรับ "รอดำเนินการ" หรืออื่นๆ
    }
  }};
`;

function QCAddModal({ isOpen, onClose, data }) {
  ...
  return (
    <Overlay onClick={onClose}>
      <ModalContent onClick={(e) => e.stopPropagation()}>
        ...
        <Row><Bold>วันที่ผลิต: </Bold> {data.prodDate}</Row>
        ...
        <Row><Bold>เครื่องจักร: </Bold> {data.machineID || '-'}</Row>
        <Row><Bold>สถานะ: </Bold>{''}
        ...
        <ButtonGroup>
          ...
          <Button color="#424242">+ เพิ่มข้อมูล</Button>
